...
REGIME_FILTER_SMA = 400 # Macro-regime filter: SMA 400 (Bull: Price > SMA, Bear: Price <= SMA)

# --- FINAL OPTIMIZED PARAMETER SETS (from Dual GA) ---

# Parameters optimized for the BULL Market (Price > SMA 400)
BULL_PARAMS = {
    'C': 0.001,
    'L': 4.6669647516892619,
    'E': 0.5586362603218745,
    'S': 0.06546073984858382,
    'P': 0.04832180195555308,
    'M': 124 
}

# Parameters optimized for the BEAR Market (Price <= SMA 400)
BEAR_PARAMS = {
    'C': 0.001,
    'L': 8.899253564170511,
    'E': 0.54840266710614572,
    'S': 0.0936572807815148,
    'P': 0.01,
    'M': 75
}
...
def run_backtest_regime_switching(df_raw):
    """
    Applies the strategy, switching parameters based on the SMA 400 regime filter.
    """
    df = df_raw.copy()
    ...
    df['Yesterday_SMA_400'] = df[f'SMA_{REGIME_FILTER_SMA}'].shift(1)
    
    # Pre-calculate strategy-specific SMAs (M=124 and M=75)
    ...
    # Drop initial NaNs (max lag is 400 + 1)
    df = df.dropna().copy()
    ...
    for i in range(len(df)):
        ...
        yesterday_sma_400 = df.loc[index, 'Yesterday_SMA_400']
        
        # --- 1. DETERMINE REGIME AND PARAMETERS ---
        # Bull Regime: Yesterday's Close Price > SMA 400
        is_bull_regime = entry_price > yesterday_sma_400
...
def plot_results(df, metrics):
    ...
    # Plot the macro regime filter SMA 400
                    ...
                    <div class="container mx-auto p-4 bg-gray-800 shadow-xl rounded-xl">
                        ...
                        <div class="mb-8">
                            ...
                            <ul class="text-xs text-gray-400 mb-4 list-disc list-inside ml-4">
                                <li>**BULL Parameters (Price > SMA {REGIME_FILTER_SMA})**: {bull_str}</li>
                                <li>**BEAR Parameters (Price &le; SMA {REGIME_FILTER_SMA})**: {bear_str}</li>
                            </ul>
