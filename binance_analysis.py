...
def create_equity_plot(results_df, long_signal_dates, short_signal_dates, horizon):
    ...
    ax1.set_yscale('log') # CRITICAL FIX
    ax1.set_title(f'BTC/USDT Price (Log Scale) - Horizon: {horizon}', fontsize=12, fontweight='bold')
    ax1.set_ylabel('Price ($)', fontsize=10)
    ax1.grid(True, which="both", ls="-", alpha=0.2)
    ax1.legend(loc='upper left')

    # Add markers for signals on the price line to ensure visibility
    # We find the price at the dates of signals
    ...
    ax2.set_yscale('log') # CRITICAL FIX
    ...
    # Add to all plots for correlation
    for date in long_signal_dates:
        ax1.axvline(x=date, color='green', linestyle='-', linewidth=0.5, alpha=0.3)
        ax2.axvline(x=date, color='green', linestyle='-', linewidth=0.5, alpha=0.3)
        ax3.axvline(x=date, color='green', linestyle='-', linewidth=0.5, alpha=0.3)
    for date in short_signal_dates:
        ax1.axvline(x=date, color='red', linestyle='-', linewidth=0.5, alpha=0.3)
        ax2.axvline(x=date, color='red', linestyle='-', linewidth=0.5, alpha=0.3)
        ax3.axvline(x=date, color='red', linestyle='-', linewidth=0.5, alpha=0.3)
