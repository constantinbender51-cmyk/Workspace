...
STATIC_CENTER_DISTANCE = 0.010 # Fixed 1.0% center distance (unmodified)
LEVERAGE_FACTOR = 5.0        # 5x Leverage
POSITION_FORMULA_EXPONENT = 0.5 # New exponent for position sizing denominator

# --- Optimized Risk Parameters (from 2D Search) ---
STOP_LOSS_PERCENT = 0.065     # 6.5% Stop Loss
REENTRY_PROXIMITY_PERCENT = 0.070 # 7.0% Proximity required to SMA 120 to re-enter
...
def run_backtest_final(df_raw):
    """
    Applies the final strategy with fixed center, 5x leverage, and optimized SL/Re-entry rules.
    """
    df = df_raw.copy()
    ...
    exponent = POSITION_FORMULA_EXPONENT
    
    print(f"-> Running final backtest (Center={center*100:.1f}%, Lev={leverage:.1f}x, SL={sl_percent*100:.1f}%, Re-entry={reentry_prox*100:.1f}%) on {len(df)} candles...")
    ...
    for i in range(len(df)):
        ...
        # Multiplier (M) calculation - APPLYING NEW EXPONENT
        ...
        if sl_cooldown:
            # If still in cooldown, force position to zero
    ...
    # Strategy
    metrics.append(generate_metrics(
        df['Cumulative_Strategy_Return'], df['Strategy_Return'], f'Dynamic Sizing (1.0% Center, {leverage:.1f}x Lev., SL {sl_percent*100:.1f}%, Re-Entry {reentry_prox*100:.1f}%)'
    ))
...
def plot_results(df, metrics):
    ...
    leverage = LEVERAGE_FACTOR
    sl_percent = STOP_LOSS_PERCENT
    ax2.set_title(f'Cumulative Return (Log Scale) - Leveraged Strategy ({leverage:.1f}x) with {sl_percent*100:.1f}% SL & {REENTRY_PROXIMITY_PERCENT*100:.1f}% Cooldown', fontsize=14, color='white')
            ...
            if self.path == '/':
                ...
                <body class="p-8">
                    <div class="container mx-auto p-4 bg-gray-800 shadow-xl rounded-xl">
                        <h1 class="text-3xl font-bold mb-4 text-green-400">Backtest Results: {SYMBOL} Dynamic Sizing ({LEVERAGE_FACTOR:.1f}x Leverage, 6.5% SL, 7.0% Re-entry)</h1>
                        
                        <div class="mb-8">
                            <h2 class="text-xl font-semibold mb-3 text-gray-200">Strategy Metrics Comparison</h2>
                            <p class="text-gray-400 mb-4">Final backtest using modified position sizing formula (exponent 0.5) and optimized risk parameters.</p>
