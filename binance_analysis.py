...
POSITION_FORMULA_EXPONENT = 0.5      # Optimal Exponent (E)
STOP_LOSS_PERCENT = 0.065            # Optimal Stop Loss (S)
REENTRY_PROXIMITY_PERCENT = 0.070    # Optimal Re-entry Proximity (P)
...
def run_backtest_final(df_raw):
    ...
    print(f"-> Running FINAL backtest (E={exponent:.1f}, SL={sl_percent*100:.1f}%, Re-entry={reentry_prox*100:.1f}%) on {len(df)} candles...")
    ...
    for i in range(len(df)):
        ...
        # Multiplier (M) calculation - USING FINAL EXPONENT
        ...
        if sl_cooldown:
            ...
            # Check for Stop Loss Trigger (if SL > 0)
    ...
    # Strategy
    metrics.append(generate_metrics(
        df['Cumulative_Strategy_Return'], df['Strategy_Return'], f'Dynamic Sizing (E={exponent:.1f}, SL {sl_percent*100:.1f}%, Re-Entry {reentry_prox*100:.1f}%)'
    ))
...
def plot_results(df, metrics):
    ...
    leverage = LEVERAGE_FACTOR
    reentry_prox = REENTRY_PROXIMITY_PERCENT
    ax2.set_title(f'Cumulative Return (Log Scale) - Optimized Strategy ({leverage:.1f}x) w/ {sl_percent*100:.1f}% SL & {reentry_prox*100:.1f}% Re-entry', fontsize=14, color='white')
                ...
                <body class="p-8">
                    <div class="container mx-auto p-4 bg-gray-800 shadow-xl rounded-xl">
                        ...
                        <div class="mb-8">
                            ...
                            <p class="text-gray-400 mb-4">Final backtest run with Exponent 0.5 and optimized risk parameters.</p>
