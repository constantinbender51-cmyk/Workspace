...
def calculate_strategy_returns(df):
    ...
    for i in range(len(df_clean)):
        ...
        stop_loss_pct = 0.15  # 15% stop loss (increased from 5%)
        
        # Condition 1: Price above both SMAs and low is 15% or more below open
        if close_price > sma_120 and close_price > sma_365:
            if low_price <= open_price * (1 - stop_loss_pct):
                stop_loss_triggered = True
        # Condition 2: Price below both SMAs and high is 15% or more above open
        elif close_price < sma_120 and close_price < sma_365:
        ...
        if stop_loss_triggered:
            df_clean['strategy_returns'].iloc[i] = -0.15  # 15% loss
        else:
