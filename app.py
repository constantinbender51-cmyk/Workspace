...
def prepare_data(df):
    ...
    df['sma_200_volume'] = df['volume'].rolling(window=200).mean()
    ...
    for i in range(len(df_clean)):
        # Features: 7-day and 14-day price SMAs, 50-day volume SMA, and 200-day volume SMA
        feature = [
            df_clean['sma_7'].iloc[i],
            df_clean['sma_14'].iloc[i],
            df_clean['sma_50_volume'].iloc[i],
            df_clean['sma_200_volume'].iloc[i]
        ]
